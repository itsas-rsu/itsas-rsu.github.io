...
const urlITSAS = import.meta.url;

export class ITSASElement extends LitElement {
    constructor() {
        super();
        ...
        this.$url = `${urlITSAS.replace('itsas-element.mjs', '')}${name}.js`;
        console.log(this.$url);
        console.log(urlITSAS);

        this.$ulid = this.$ulid || ITSAS.ulid();
        if (this._useInfo) this.$urlInfo = `${urlITSAS.replace('itsas-element.mjs', '')}/${name}/$info/$info.js`;
    }
    ...
    connectedCallback() {
        ...
        if (this.$$ && this.__globals) {
            ITSAS.$$.listen(this.fnGlobals);
            this.__globals.forEach(i => {
                if (ITSAS.$$[i] === undefined) ITSAS.$$[i] = this[i];
                else this[i] = ITSAS.$$[i];
            });
    ...
    disconnectedCallback() {
        ...
        if (ITSAS.$$ && this.__globals) ITSAS.$$.unlisten(this.fnGlobals);
    ...
    _initBus() {
        if (this.$partid || this.$properties.get('$partid') || (!this.$$ && (this.$properties.get('_partid') || this.__saves || !this.$root || this.__locals || this.__globals))) {
            ...
            if (!ITSAS._$$[this._partid]) {
                ITSAS._$$[this._partid] = { _$$: {}, _$$: {} };
                ITSAS._$$[this._partid]._$$ = icaro({});
                ITSAS._$$[this._partid]._$$.update = icaro({ value: 0 });
            }
        }
    }
    ...
    get $$() { return ITSAS._$$?.[this.partid]?.['_$$'] ? ITSAS._$$[this.partid]['_$$'] : undefined }
        ...
        for (const prop of changedProps.keys()) {
            ...
            if (this.__isFirstUpdated) {
                ...
                if (this.$$ && this.__globals && this.__globals.includes(prop))
                    ITSAS.$$[prop] = this[prop];
                // if (ITSAS._changed) console.log('_changed', this.localName, prop, { old: changedProps.get(prop) }, { new: this[prop] });
                // if (ITSAS._$update) ++this.$$.__changed;
            }
            if (this.__notifications && this.__notifications.has(prop)) {
                ...
                //if (ITSAS._notify) console.log('_notify ', this.localName, event, { old: changedProps.get(prop) }, { new: this[prop] });
            }
        }
    }

    $update(property, value) { ITSAS.$update.call(this, property, value) }
    $listen(event, fn) {
        if (!event) return;
        ITSAS.$$[event] = ITSAS.$$[event] || icaro({ count: 0 });
        ITSAS.$$[event].listen(fn || this.fnListen);
    }
    $unlisten(event, fn) {
        if (!event || !ITSAS.$$[event]) return;
        ITSAS.$$[event].unlisten(fn || this.fnListen);
        ITSAS.$$[event] = undefined;
    }
    $fire(event, value) {
        if (!event) return;
        ITSAS.$$[event] = ITSAS.$$[event] || icaro({ count: 0 });
        ITSAS.$$[event].value = value;
        ++ITSAS.$$[event].count;
    }
...
class CITSAS {
    ...
    async createComponent(comp, props = {}) {
        ...
        if (typeof comp === 'string') {
            ...
            let url = `${urlITSAS.replace('js/itsas-element.mjs', '')}components/${comp}/${comp}.js`;
            await import(url);
...
document.addEventListener('mousedown', (e) => ITSAS.mousePos = new DOMRect(e.pageX, e.pageY));
if (!window.ITSASRect) {
    window.ITSASRect = function(element) {
        if (element && element.host) element = element.host;
        const pos = element ? element.getBoundingClientRect() : ITSAS.mousePos;
        return pos ? {
